...
> **このページは何か(かみ砕いた説明)**: llove という道具をお試しモードで動かすと、いろいろな画面が出ます。このページは、その画面を 1 つずつ写真に撮って並べた「アルバム」です。動物園の動物を 1 種類ずつ写真にしてアルバムに貼っていくのと同じ感覚で、機能ごとに 1 枚ずつためていきます。写真は、どれだけ大きく拡大しても文字がぼやけない種類の画像を使っています。同じ画面を日本語のものと英語のものの 2 枚ずつ並べ、時間とともに変わっていく画面はパラパラ漫画のように動く画像で残しています。むずかしい言葉が出てきたら、言葉の説明をまとめた[用語集](../GLOSSARY.md)を見てください。
...
- `App.save_screenshot()` / `App.export_screenshot()` は Textual 標準機能、
  ターミナル非依存の SVG を生成
- LLM 呼び出しを必要とする シナリオ(scenario) (chat / multimodal) は `MockBackend` 前提

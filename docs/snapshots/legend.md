# llove TUI(Text User Interface、テキストユーザーインターフェース) レイアウト凡例

> **かみ砕いた説明**: このページは、llove のデモ画面を写した絵の「見方の地図」です。ちょうど、遊園地でもらう案内図のように、画面のどこが押せるボタンで、どこが見るだけの場所(押しても動かない表示エリア)なのかを、枠線と小さな絵記号で教えてくれます。さらに、それぞれの場所が何を表しているかを一覧表にまとめています。画面の絵を見ながら、「どこを押せるのか」「どこから何が読み取れるのか」を確かめるための早見表だと思ってください。
>

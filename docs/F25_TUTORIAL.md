...
> **かみ砕いた説明**: スポーツの試合を「電光掲示板」に映すところを想像してください。選手(ここでは llove の相棒の道具)が動くと、その様子が掲示板にすぐ表示されます。このチュートリアルは、その掲示板を文字だけの黒い画面の中に作る手順を、やさしい順に 3 段階で教えます。最初は本番の道具につながず、にせ物のデータで練習する「自主練モード」から。次に道具を 1 つだけつないだ「半分本番」。最後に全部つないだ「本番」へと進みます。途中で意味のわからない言葉が出てきたら、巻物のように言葉の意味をまとめた [用語集(GLOSSARY.md)](./GLOSSARY.md) を見てください。

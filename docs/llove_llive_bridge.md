...
> **かみ砕いた説明:** この文書は、FullSense ™ の 3 つの道具 (llmesh / llive / llove) を 1 本のホースでつなぐ配線図です。たとえると、台所 (llive) で出てきた料理の記録 (どんな手順で作ったか・何を覚えたか・味くらべの結果) を、まんなかの受け渡し台 (llmesh) を通して、食卓の画面 (llove) まで運ぶ、というイメージです。
>
> ここでの工夫は「新しい運び役を作らない」ことです。まんなかの受け渡し台に「データを受け取る小窓」を 1 つ足すだけで、3 つの道具がつながります。大きな部品を増やさず、いちばん少ない手間でつなぐ設計をまとめたのがこの文書です。
>
> このあと出てくる短いアルファベット (MCP や TUI など) の意味は、まとめのページ [GLOSSARY.md](GLOSSARY.md) で説明しています。そこを見れば一つずつ確かめられます。

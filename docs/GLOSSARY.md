...
llove（ラブ）は、**黒い文字だけの画面（文字を打って操作する画面）の中で、グラフや表をきれいに動かして見せてくれる道具**です。車の運転席の前にある、速度やガソリンの残りを一目で見られる計器盤を思いうかべてください。あの計器盤の、画面版のようなものです。

たとえば、工場の機械についた測定器（温度や振動などをはかる部品）の数字を、まとめて流し見できます。人の言葉を理解して答えてくれる AI の動きも、同じ 1 つの画面で見られます。気になる画面が出たら、ボタン 1 つで、いま見ている画面をそのまま 1 個のファイルに保存できます。そのファイルを仲間にメッセージや資料で送って、共有できます。

それだけでなく、大事な場面で人が「これでいいよ」と確認しながら、一緒に作業を進める作業台としても使えます。llove は、3 つの製品の家族（llmesh・llive・llove）の 1 つです。この家族の中で llove は、**画面に見せる役と、人の確認を受け持つ役**を担当します。
